...
void AllocStrs(void)
{
  if (!gptVarName) gptVarName = StrAlloc(g_stringsize+APPEND_SIZE);
  if (!gptPathString) gptPathString = StrAlloc(g_stringsize+APPEND_SIZE);
  if (!gptBuffer) gptBuffer = StrAlloc(g_stringsize+APPEND_SIZE);
}
